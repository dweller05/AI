...
class Agent:
    ...
    def get_state(self, game):
        ...
        # point_l2 = Point(tail.x - 20, tail.y)
        # point_r2 = Point(tail.x - 20, tail.y)
        # point_u2 = Point(tail.x, tail.y - 20)
        # point_d2 = Point(tail.x, tail.y - 20)
    ...
    def get_action(self, state):
        ...
        if random.randint(0, 200) < self.epsilon:
            ...
            prediction = self.model(state0) # This can be a raw value
...
def train():
    ...
    while True:
        ...
        if done:
            # train long memory(replay memory, experienced memory), plot result
            game.reset()
            agent.n_games += 1
